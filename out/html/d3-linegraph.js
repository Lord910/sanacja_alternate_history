...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['spd', 'kpp', 'psl-w', 'z', 'psl-p', 'sn', 'owp', 'other'];
    }
    if (!partyColors) {
        partyColors = {'spd': '#7a7a7a', 'kpp': '#CF3535', 'psl-w': '#62DC4A', 'z': '#000', 'psl-p': '#D5AC27', 'sn': '#3f7bc1', 'owp': '#954B00', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'spd': 'SPD', 'kpp': 'kpp', 'psl-w': 'PSL-W', 'z': 'Z + BVP', 'psl-p': 'PSL-P', 'sn': 'SN', 'owp': 'OWP', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
